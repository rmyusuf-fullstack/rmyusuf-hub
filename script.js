...
document.querySelectorAll(".yt-player").forEach((player) => {
  ...
  const carouselItem = player.closest(".carousel-item");

  if (carouselItem) {
    watch(player, "reveal-video", carouselItem);
    return;
  }
...
function positionCarouselArrows(state) {
  const top = state.viewport.offsetTop + state.viewport.clientHeight / 2;

  state.previousButton.style.top = `${top}px`;
  state.nextButton.style.top = `${top}px`;
}

function updateCarouselArrows(state) {
  const { viewport, previousButton, nextButton } = state;
...
function initializeCarousel(carousel) {
  ...
  window.addEventListener("resize", () => positionCarouselArrows(state));

  viewport.addEventListener(
    "scroll",
    () => {
      if (!state.isNudging && viewport.scrollLeft > 1) state.nudged = true;

      if (state.scrollFrame) return;
      state.scrollFrame = window.requestAnimationFrame(() => {
        state.scrollFrame = 0;
        updateCarouselArrows(state);
      });
    },
    { passive: true }
  );

  positionCarouselArrows(state);
  updateCarouselArrows(state);
}

document.querySelectorAll("[data-carousel]").forEach(initializeCarousel);

const carouselNudgeObserver = new IntersectionObserver(
  (entries, observer) => {
    entries.forEach((entry) => {
      if (!entry.isIntersecting) return;
      ...
      const canScroll = state && state.viewport.scrollWidth > state.viewport.clientWidth;

      if (!state || state.nudged || !canScroll) {
      ...
      const item = state.track.querySelector(".carousel-item");
      const distance = item.getBoundingClientRect().width * 0.35;
      state.isNudging = true;
      state.viewport.scrollBy({ left: distance, behavior: "smooth" });

      window.setTimeout(() => {
        state.viewport.scrollTo({ left: 0, behavior: "smooth" });
...
document.querySelectorAll("[data-carousel]").forEach((carousel) => {
  carouselNudgeObserver.observe(carousel);
});
